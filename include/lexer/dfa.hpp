...
struct DFA {
    ...
    DFA(int num_states, int start_state, const std::vector<std::vector<int>>& delta, const std::vector<Token> token_types): num_states(num_states), start_state(start_state), delta(delta), token_types(token_types) {}
    DFA minimize() const {
        ...
        for (int i = 0; i < num_states; i++) {
            if (visited[i]) mapping[i] = cnt++;
        }
        int start_state_updated = mapping[start_state];
        std::vector<std::vector<int>> delta_updated(cnt, std::vector<int>(ALPHABET_SIZE));
        for (int i = 0; i < num_states; i++) {
            for (int alpha = 0; alpha < ALPHABET_SIZE; alpha++) {
                if (mapping[i] != -1) {
        ...
        do {
            ...
            for (int i = 0; i < cnt; i++) {
                for (int j = 0; j < cnt; j++) {
                    for (int alpha = 0; alpha < ALPHABET_SIZE; alpha++) {
                        if (!table[i][j] && table[delta_updated[i][alpha]][delta_updated[j][alpha]]) {
                            table[i][j] = 1;
                            table[j][i] = 1;
                            change = 1;
                        }
        ...
        std::vector<std::vector<int>> delta_new(num_states_new, std::vector<int>(ALPHABET_SIZE));
        for (int i = 0; i < cnt; i++) {
            for (int alpha = 0; alpha < ALPHABET_SIZE; alpha++) {
                delta_new[mapping_new[dsu.find(i)]][alpha] = mapping_new[dsu.find(delta_updated[i][alpha])];
            }
        }
        std::vector<Token> token_types_new(num_states_new, Token::TOK_ERROR);
        for (int i = 0; i < cnt; i++) {
            token_types_new[mapping_new[dsu.find(i)]] = max(token_types_new[mapping_new[dsu.find(i)]], token_types_updated[i]);
        }
