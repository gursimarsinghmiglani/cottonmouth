...
inline bool concat_valid(const std::string& regex, int i, bool escaped) {
    return (escaped || regex[i] == '*' || regex[i] != '\\' && !is_operator(regex[i]) || regex[i] == ')') && (!is_operator(regex[i + 1]) || regex[i + 1] == '(');
}
...
inline NFA regex_to_nfa(const std::string& regex) {
    if (regex.empty()) {
        std::vector<std::vector<std::vector<int>>> delta(1, std::vector<std::vector<int>>(ALPHABET_SIZE + 1));
    ...
    for (int i = 0; i < postfix_regex.size(); i++) {
        if (is_operator(postfix_regex[i]) && !escaped) {
            ...
            if (postfix_regex[i] == 'e' && escaped) {
                ...
                std::vector<std::vector<std::vector<int>>> delta2(1, std::vector<std::vector<int>>(ALPHABET_SIZE + 1));
                ...
                std::vector<std::vector<std::vector<int>>> delta2(2, std::vector<std::vector<int>>(ALPHABET_SIZE + 1));
...
inline DFA regex_to_dfa(const std::vector<std::string>& regexes, const std::vector<Token>& tokens) {
    std::vector<std::vector<std::vector<int>>> delta(1, std::vector<std::vector<int>>(ALPHABET_SIZE + 1));
