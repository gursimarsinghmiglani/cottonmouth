...
struct NFA {
    ...
    NFA(int num_states, int start_state, const std::vector<std::vector<std::vector<int>>>& delta, std::vector<Token> token_types): num_states(num_states), start_state(start_state), delta(delta), token_types(token_types) {}
    NFA union_nfa(const NFA& other) const {
        ...
        std::vector<std::vector<std::vector<int>>> delta_new(num_states_new, std::vector<std::vector<int>>(ALPHABET_SIZE + 1));
        delta_new[0][ALPHABET_SIZE] = {start_state + 1, other.start_state + num_states + 1};
        for (int i = 0; i < num_states; i++) {
    ...
    NFA kleene_star() const {
        ...
        std::vector<std::vector<std::vector<int>>> delta_new(num_states + 1, std::vector<std::vector<int>>(ALPHABET_SIZE + 1));
        for (int i = 0; i < num_states; i++) {
            for (int alpha = 0; alpha < ALPHABET_SIZE + 1; alpha++) {
                for (int v: delta[i][alpha]) {
                    delta_new[i][alpha].push_back(v);
    ...
    DFA to_dfa() const {
        ...
        for (const auto& pair: Q) {
            for (int x: pair.first) {
                token_types_dfa[pair.second] = max(token_types_dfa[pair.second], token_types[x]);
            }
        }
        std::vector<std::vector<int>> delta_dfa(cnt, std::vector<int>(ALPHABET_SIZE));
        for (int i = 0; i < cnt; i++) {
            for (int alpha = 0; alpha < ALPHABET_SIZE; alpha++) {
